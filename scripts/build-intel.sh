...
# Use /tmp for shim to avoid path issues with spaces in WORK_DIR.
if [[ ! -f "/Library/Developer/CommandLineTools/usr/include/c++/v1/source_location" ]] && \
  ...
  SHIM_INCLUDE_DIR="/tmp/codex-intel-shim-$$"
...
(
  cd "${BUILD_PROJECT}"
  # Build rebuild command with optional shim flags
  if [[ "${SHIM_NEEDED:-0}" == "1" ]]; then
    run_with_estimated_progress 75 84 1800 "Rebuilding native modules for Electron ${ELECTRON_VERSION} x64" \
      run_with_timeout 1800 env \
        CPPFLAGS="-I/tmp/codex-intel-shim-$$" \
        CXXFLAGS="-I/tmp/codex-intel-shim-$$" \
